...
ggsave("Bleaching_Treatment x Primed x Date.tiff", width = 18, height = 18, units = "cm", dpi=1200, compression = "lzw")
...
ggplot(data=sum.HTD, aes(Date, y = Brightness, fill = Treatment)) +
  ...
  theme(
    ...
    axis.text.y = element_text(size = 9, face = "bold", vjust = 0.4),
    strip.text = element_text(size = 11))
ggsave("Bleaching_Heatprimed x Treatment x Date.tiff", width = 18, height = 18, units = "cm", dpi=1200, compression = "lzw")
...
ggplot(data = letters.GHT, aes(y = response, x = Treatment, fill = Position, pattern = Heatprimed,
  ...
  labs(y = "Brightness (bpp)", x = "Treatment", fill = "Genotype")+
...
ggplot(df5.complete, aes(x = `Brightness (priming)`, y = `Brightness (natural bleaching)`, colour = Treatment))+
  ...
  geom_smooth(  method = "lm", se = F, size = 1.2, fullrange = F)+ 
  labs(x = "Brightness priming (bpp)", y = "Brightness natural heatwave (bpp)")+
  theme(
...
## Plot averages of bleaching response during priming
df5.complete <- dplyr::select(df5.complete, -c("Structure", "Position", "finder" ))
names(df5.complete) <- c("Treatment", "Brightness_1", "Brightness_2")
df5.complete <- as.data.frame(long_panel(df5.complete, prefix = "_", begin = 1, end = 2, label_location = "end", check.varying = FALSE))
df5.complete$wave <- ifelse(df5.complete$wave == 1, "Priming", "Heatwave")
df5.complete <- dplyr::select(df5.complete, -c("id" ))
names(df5.complete) <- c("Time", "Treatment", "Brightness.max")
df5.complete$Treatment <- as.factor(df5.complete$Treatment)
df5.complete$Time <- as.factor(df5.complete$Time)

### Model  
Model.bleach.time <- lm(log10(Brightness.max) ~ Treatment*Time, data = df5.complete)

anova(Model.bleach.time) # Three-way interaction highly significant also for Genotype (Position)

### Residuals
mod <- Model.bleach.time # set model to be validated
op <- par(mfrow = c(2, 3), mar = c(5, 4, 1, 2)) # set layout of figure frame
plot(resid(mod, type = "pearson") ~ fitted(mod)) # fitted vs residuals
abline(0,0)
hist(resid(mod, type = "pearson"), main = "") # histogram residuals
qqnorm(resid(mod, type = "pearson")) # QQplot residuals
plot(df5.complete$Treatment, resid(mod, type = "pearson")) # residuals split over Treatment
abline(0,0)
plot(df5.complete$Time, resid(mod, type = "pearson")) # residuals split over Heatprimed
abline(0,0)
plot(fitted(mod) ~ log10(df5.complete$Brightness.max)) # response (log) data vs fitted (log)
par(op)

### Posthoc
ph.bleach.time <- emmeans(Model.bleach.time, specs = pairwise ~ Treatment|Time, adjust = "tukey", type = "response")
letters.bleach.time <- multcomp::cld(ph.bleach.time$emmeans, alpha = 0.05, Letters = letters) # get CLD
letters.bleach.time <- letters.bleach.time %>% mutate(.group = str_squish(.group)) # remove white spaces

### Plot
ggplot(data = letters.bleach.time, aes(y = response, x = Treatment,  fill = Treatment, pattern = Time, pattern_spacing = Time, 
                          pattern_density = Time))+
    geom_hline(yintercept=90, linetype="twodash", color = "#f38aff", size = 0.55)+
  geom_hline(yintercept=150, linetype="dotted", color = "#f38aff", size = 0.75)+
  geom_col_pattern(stat = "identity", position = position_dodge(.9), width = .85,
      pattern_colour = 'black', pattern_fill = 'black', colour = 'black')+
  ...
  labs(y = "Brightness (bpp)", x = "Treatment", pattern = "", pattern_spacing = "", pattern_density = "")+
  scale_y_continuous(expand = c(0, 0), breaks = c(40, 80, 120, 160), limits = c(0, 165))+
  geom_errorbar(aes(ymin = response - (1*SE), ymax = response + (1*SE)), width=.1, position=position_dodge(.9))+
  geom_text(data = letters.bleach.time, aes(x = Treatment, y = response + 1*SE, vjust = -1, label = .group), 
  ...
  theme(
    ...
    axis.text.y = element_text(size = 9, face = "bold", vjust = 0.4),
    strip.text = element_text(size = 11))
ggsave("Bleaching_Brightness by Priming vs Natural heatwave.tiff", width = 9, height = 10, units = "cm", dpi=1200, compression = "lzw")
